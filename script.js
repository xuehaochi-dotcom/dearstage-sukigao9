...
function makeQual1Blocks(){
  ...
  for(let i = 0; i < shuffled.length; i += 4){
    state.qual1Blocks.push(shuffled.slice(i, i + 4));
  }
}
...
function renderQual1(){
  const block = state.qual1Blocks[state.qual1Index];

  $("#qual1Title").textContent =
    `予選1 ${state.qual1Index + 1} / ${state.qual1Blocks.length}`;
  ...
  block.forEach(person => {
    ...
    card.innerHTML = `
      <img src="${imageFor(person)}" alt="${person.name}">
      <div>${person.name}</div>
    `;
    ...
    card.onclick = () => {
      if(state.qual1Selected.has(person.id)){
        ...
        if(state.qual1Selected.size >= 2) return;
...
function finishQual1Block(){
  ...
  block.forEach(person => {
    if(state.qual1Selected.has(person.id)){
      if(!state.qual1Winners) state.qual1Winners = [];
  ...
  if(state.qual1Index < state.qual1Blocks.length){
    state.qual1Selected = new Set();
    renderQual1();
  }else{
    prepareQual2();
...
function prepareQual2(){
  state.qual1Winners = shuffle(state.qual1Winners);

  state.qual2Blocks = [];

  for(let i = 0; i < state.qual1Winners.length; i += 5){
  ...
  $("#betweenEyebrow").textContent = "30 → 12";
  $("#betweenTitle").textContent = "SECOND QUALIFYING";
  $("#betweenText").textContent =
    "予選1を通過した30人から、さらに12人へ。5人の中から好きな2人を選んでください。";
  $("#betweenBtn").textContent = "SECOND ROUND START";

  show("#screen-between");

  $("#betweenBtn").onclick = startQual2;
}
...
function renderQual2(){
  const block = state.qual2Blocks[state.qual2Index];

  $("#qual2Title").textContent =
    `予選2 ${state.qual2Index + 1} / ${state.qual2Blocks.length}`;
  ...
  block.forEach(person => {
    ...
    card.onclick = () => {
      if(state.qual2Selected.has(person.id)){
        ...
        if(state.qual2Selected.size >= 2) return;
  ...
  $("#qual2Count").textContent = "0 / 2";
  $("#qual2NextBtn").disabled = true;
}
...
function finishQual2Block(){
  ...
  block.forEach(person => {
    if(state.qual2Selected.has(person.id)){
      if(!state.qual2Winners) state.qual2Winners = [];
  ...
  if(state.qual2Index < state.qual2Blocks.length){
    state.qual2Selected = new Set();
    renderQual2();
  }else{
    prepareFinal();
...
function prepareFinal(){
  state.finalists = shuffle(state.qual2Winners);

  state.finalScores = new Map(
    state.finalists.map(p => [p.id, 0])
  );

  state.finalPairs = roundRobin(state.finalists);
  state.finalIndex = 0;

  $("#betweenEyebrow").textContent = "12 → 9";
  $("#betweenTitle").textContent = "FINAL ROUND";
  $("#betweenText").textContent =
    "12人の完全総当たり。66試合を通して、あなたの好きなお顔TOP9を決めます。";
...
function updateMatchUI(){
  const pair = state.finalPairs[state.finalIndex];
  const total = state.finalPairs.length;

  $("#phaseLabel").textContent = "FINAL / ROUND ROBIN";
  $("#matchTitle").textContent = "12人・完全総当たり";
  $("#counter").textContent =
